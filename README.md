...
- **Full demo dataset** (`scripts/seed_demo_data.php`) — see step 5
  ...
  shown alongside. Browse renders as a photo-forward catalog grid
  (Shopee-style: photo on top, details below), 3 cards per row on
  desktop, 2 on tablet widths, and 1 per row on phone widths — the same
  breakpoint that will apply inside the future Android WebView wrapper.
...
`scripts/generate_icons.php` builds two separate icon sets from two
different sources, per the project owner's instruction — the website's
own identity (favicon, apple-touch-icon, manifest icon-192/512) is a
rendered 🧹 emoji (`assets/emoji-source.png`), while the Android APK's
launcher icon (`assets/icons/apk-icon-*.png`, used only by
`scripts/build_twa.js`, not referenced by `manifest.json` at all) is the
real logo (`assets/logo-source.png`). Re-run whenever either source
image changes; PHP's GD extension isn't enabled by default in a stock
XAMPP install, so run it with `-d extension=php_gd.dll` rather than
editing `php.ini`:
```bash
php -d extension=php_gd.dll scripts/generate_icons.php
```

## Android APK (test build, not Play Store)

A sideloadable Android APK is generated by wrapping the PWA above as a
[Trusted Web Activity](https://developer.chrome.com/docs/android/trusted-web-activity)
via Google's [Bubblewrap](https://github.com/GoogleChromeLabs/bubblewrap)
tooling — this reuses the manifest/service-worker already in place
rather than hand-writing a native WebView Activity. Scope is deliberately
narrow for now: a signed APK you can install directly on a test device.
No Play Store account, store listing, or push notifications yet. The
launcher icon does **not** come from `manifest.json` — `build_twa.js`
overrides it to point at `assets/icons/apk-icon-512.png` (the real logo)
so the installed app icon differs from the website's emoji identity.

Toolchain needed (already set up on the dev machine used for this
project, documented here for a fresh machine):
- **JDK 17** — Bubblewrap requires exactly this version (older fails to
  compile, newer breaks the Android command-line tools). Not the same
  JDK PHP/other tooling might use.
- **Android SDK command-line tools** — platform-tools, a recent
  `build-tools` version, and a matching `platforms;android-XX`, installed
  via `sdkmanager`.
- **`@bubblewrap/cli`** — `npm install -g @bubblewrap/cli` (Node.js
  already required for nothing else in this PHP project — this is the
  only reason Node shows up here at all).

To (re)build after the live URL changes (e.g. a new Cloudflare Quick
Tunnel hostname):
```bash
node scripts/build_twa.js "https://<current-tunnel-url>/MaidTrackingApps/manifest.json"
cd <twa-project-directory>
BUBBLEWRAP_KEYSTORE_PASSWORD=<pw> BUBBLEWRAP_KEY_PASSWORD=<pw> node <path-to>/scripts/sign_apk.js
```
Both scripts drive `@bubblewrap/core` directly rather than the
interactive `bubblewrap` CLI wizard, since every value it would ask about
is already decided. See `CREDENTIALS.md` (gitignored) for the actual
keystore location and password used on this machine.

**Known limitation**: without a stable, domain-verified
`assetlinks.json`, the installed app shows a browser URL bar instead of
being fully "app-like" — expected for a build tied to the ephemeral
Cloudflare Quick Tunnel URL rather than a stable domain. Resolves once
Phase 5's hosting is stable (Named Tunnel or actual cPanel).
