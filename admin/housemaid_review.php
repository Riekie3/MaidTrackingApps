...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'approve') {
        ...
        flash('success', $hm['full_name'] . ' approved.');
    } elseif ($action === 'reject') {
        $reason = trim($_POST['reason'] ?? '');
        if ($reason === '') {
            flash('error', 'A rejection reason is required.');
            redirect(rtrim(APP_URL, '/') . '/admin/housemaid_review.php?id=' . $id);
        }
        Housemaid::reject($id, $adminId, $reason);
        flash('success', $hm['full_name'] . ' rejected.');
    }
    redirect(rtrim(APP_URL, '/') . '/admin/agency_housemaids.php?agency_id=' . $hm['agency_id']);
}
...
<div class="container">
    <div class="page-head">
        <div>
            ...
            <p>
                Submitted by <strong><?= e($hm['agency_name']) ?></strong> ·
                <span class="pill <?= e($hm['approval_status']) ?>"><?= e(ucfirst($hm['approval_status'])) ?></span>
            </p>
        </div>
        <a class="btn btn-outline" href="<?= e(rtrim(APP_URL, '/')) ?>/admin/agency_housemaids.php?agency_id=<?= (int) $hm['agency_id'] ?>">← Back to <?= e($hm['agency_name']) ?></a>
    </div>
    ...
    <div class="card" style="margin-bottom:20px;">
        ...
        <div class="detail-grid">
            ...
            <div class="detail-item"><div class="dl">Religion</div><div class="dv"><?= e($hm['religion'] ?? '—') ?></div></div>
    ...
    <div class="card" style="margin-bottom:20px;">
        ...
        <div class="detail-grid">
            <div class="detail-item"><div class="dl">Passport number</div><div class="dv mono"><?= e($hm['passport_number'] ?? '—') ?></div></div>
            <div class="detail-item"><div class="dl">Passport expiry</div><div class="dv"><?= fmt_date($hm['passport_expiry']) ?></div></div>
            <div class="detail-item"><div class="dl">Work permit number</div><div class="dv mono"><?= e($hm['work_permit_number'] ?? '—') ?></div></div>
            <div class="detail-item"><div class="dl">Work permit expiry</div><div class="dv"><?= fmt_date($hm['work_permit_expiry']) ?></div></div>
            <div class="detail-item"><div class="dl">National ID</div><div class="dv mono"><?= e($hm['national_id_number'] ?? '—') ?></div></div>
        </div>
    ...
    <div class="card" style="margin-bottom:20px;">
        ...
        <div class="detail-grid">
            ...
            <div class="detail-item"><div class="dl">PDPA consent</div><div class="dv"><?= $hm['consent_given_at'] ? 'Given ' . fmt_date($hm['consent_given_at']) : '—' ?></div></div>
        </div>
    </div>

    <?php if ($hm['approval_status'] === 'pending'): ?>
    <div class="card">
        <h2>Decision</h2>
        <div class="btn-row" style="margin-bottom:16px;">
            <form method="post" data-confirm="Approve <?= e($hm['full_name']) ?>?">
                <?= csrf_field() ?>
                <input type="hidden" name="action" value="approve">
                <button type="submit" class="btn btn-primary">Approve</button>
            </form>
        </div>
        <form method="post" data-confirm="Reject <?= e($hm['full_name']) ?>?">
            <?= csrf_field() ?>
